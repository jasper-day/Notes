...
which means that $\delta \propto \root \of x$ (since $Re_x \propto x$), and 
$\delta \propto \frac{1}{\root \of {u_\infty}}$ (since $Re_x \propto u$). That is to say, the boundary layer increases with the square root of the distance from the leading edge, and the boundary layer thickness increases with one over the square root of the free stream velocity.
...
The boundary layer thickness with turbulent flow increases with $x^{4/5}$, and it increases with $1/(\root^{1/5} \of {u_\infty})$). This is much closer to a straight line in x.
...
$$
F_dD = \frac{1}{2} C_f \, \rho \, A \, u_\infty^2
$$
